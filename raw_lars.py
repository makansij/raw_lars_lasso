    ...
    for i in xrange(len(feature_cols)):
        if i==0:
            ...
            while (not better_corr):
                ...
                if 'x7' in current_lm_model.coeff_dict.keys() and k%1000==0:
                    print ' curr_model_correlation is ', curr_model_correlation
                    print ' maximum_correlation is ', maximum_correlation
                if maximum_correlation[0] > curr_model_correlation:
                    ...
                    better_corr=True
                    models = models + [copy.deepcopy(current_lm_model)]
                    print 'models is ', [model.coeff_dict for model in models]
                else:
                    ...
                    eps[index] = 10.0                     # go to that position in the empty array above, and replace it with epsilon   
                    x += eps                              # add the array with epsilon but otherwise empty, to the original array, x
                    # print ' the beta vector was increased to ', x
                    # print ' after it is ', dict(zip(keys,x))
                    # print ' current_lm_model.coeff_dict.items()[:4] should be the same \n', current_lm_model.coeff_dict.items()[:4]
                    k = k+1
                    current_lm_model.coeff_dict = dict(zip(keys,x))
                    if 'x7' in current_lm_model.coeff_dict.keys() and k%100==0:
                        print 'the dictionary after k = ', k, ' iterations is ', current_lm_model.coeff_dict
...
global models  # for debugging purposes 
models = []
